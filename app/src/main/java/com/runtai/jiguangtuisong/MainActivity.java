package com.runtai.jiguangtuisong;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.init:
                ...
                break;
            case R.id.setting:
                Intent intent = new Intent(MainActivity.this, PushSetActivity.class);//跳转到设置服务的页面
                startActivity(intent);
                break;
            case R.id.stopPush:
                JPushInterface.stopPush(getApplicationContext());//推送服务
                break;
            case R.id.resumePush:
                JPushInterface.resumePush(getApplicationContext());//恢复推送服务
                break;
            case R.id.getRegistrationId:
                String rid = JPushInterface.getRegistrationID(getApplicationContext());//SDK 向 JPush Server 注册所得到的注册 ID
                if (!rid.isEmpty()) {
                    mRegId.setText("RegId:" + rid);
                } else {
                    Toast.makeText(this, "没有获取到ID，注册失败", Toast.LENGTH_SHORT).show();
                }
                break;
        }
    }
